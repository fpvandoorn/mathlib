...
Tested to compile on branch d4343972 of mathlib.
Skip to the line with `structure cube` for the material specific to this formalization.
-/
...
@[simp] lemma add_le_self_left : x + y ≤ x ↔ y ≤ 0 :=
by { convert add_le_add_iff_left x, rw [add_zero] }

@[simp] lemma add_lt_self_right : x + y < y ↔ x < 0 :=
by { convert add_lt_add_iff_right y, rw [zero_add] }

@[simp] lemma add_lt_self_left : x + y < x ↔ y < 0 :=
by { convert add_lt_add_iff_left x, rw [add_zero] }
-- the four dual ones are already in mathlib, e.g. lt_add_iff_pos_right
end
-- end move
...
begin
  have h' := tail_sub hi j, dsimp [side] at h', rw [Ico_subset_Ico_iff] at h',
  exact h'.2, simp [hw]
end
...
begin
  ...
  { rcases (two_le_iff' (⟨i, hi⟩ : bcubes cs c)).mp (two_le_mk_bcubes h v) with ⟨⟨i', hi'⟩, h2i'⟩,
    refine ⟨⟨i', hi', _⟩⟩, simp [mem_singleton_iff], rintro h, apply h2i', simp [h] },
  ...
  { dsimp only [x], rw [←bi, add_sub_assoc, add_lt_self_left, sub_lt_zero],
    apply mi_strict_minimal (ne.symm h2i') hi' },
  refine ⟨x, ⟨_, _⟩, _⟩,
  { simp [side, x, -add_comm, -add_assoc, neg_lt_zero, hw],
    rw [add_assoc, le_add_iff_nonneg_right, ←sub_eq_add_neg, sub_nonneg],
    apply le_of_lt (w_lt_w h v hi') },
  { simp [side, not_and_distrib], left, exact hx },
